...
if gui then vim.o.clipboard = 'unnamedplus' end
...
vim.api.nvim_create_user_command('SetIndent', function(opts)
  ...
  if indent == nil then error 'indent is not an int' end
  set_indent(indent)
end, { nargs = '?', desc = 'Set indent len' })
...
vim.fn['plug#begin']()
  ...
  if completion == 'copilot' then vim.cmd "Plug 'github/copilot.vim'" end
vim.fn['plug#end']()
...
vim.keymap.set('n', '<A-1>', function() vim.api.nvim_command 'NERDTreeToggle' end)

vim.keymap.set({ 'n', 'i' }, '<C-A-L>', function() vim.api.nvim_command 'Neoformat' end)
